...
void stringToHex(const char *input, char *output, size_t size)
{
    ...
    while (*input) {
        if (*input == '\\'){
            if(!(*input)) puts("stringToHex: No se a finalizado la cadena");
            switch (*(input+1))
            {
            case '0' :sprintf(output, "%02x", '\0');break;
            case 'r' :sprintf(output, "%02x", '\r');break;
            case 'n' :sprintf(output, "%02x", '\n');break;
            case 't' :sprintf(output, "%02x", '\t');break;
            case '\\':sprintf(output, "%02x", '\\');break;
            default: sprintf(output, "%02x", *(input+1));  // Convierte el caracter a formato hexadecimal
            }
            input+=2;
        }
        else{
...
void convert_string_to_push_reverse(list_c *list, size_t frame, name_value *value)
{
    ...
    list_c *list_temp = init_list(sizeof(unsigned char)); // lista termporal
    for (size_t i = size_str; i > 0; i -= size_word)
    {
        if ( i > SIZE_MAX -0xff ) break; // al ser un valor unsigned al pasar el 0 pasa a su valor maximo
        ...
        printf("%lld %s\n",  real_size_hex,instrucciones);
        list_push(list_temp, instrucciones);
        /*switch (compiler_word_arch)
        {
        case 64:
    ...
    {
        list_push(list, list_temp->items[i]);
        printf("%zu %s\n", i, (unsigned char*)list_temp->items[i]);
    }
    list_push(list, list_temp->items[0]);
